...
func aboutStrings() {
	...
	//looks like chars are with single quotes
	...
	//chars can be concatenated

	bytes[0] = 'z'
	assert(string(bytes) == "zbc") // byte-slices can be mutated, although strings cannot
	//can mutate the byes array to change the output string
	// mutate_string := "zbc"
	// mutate_string[0] = 'a'
	//error is : 'cannot assign to mutate_string[0] (neither addressable nor a map index expression)'
	...
	//%d for digit, %f for float - 0.2 for 2 dec places
}
